...
if __name__ == "__main__":
    ...
    fig2, ax2 = plt.subplots(nrows=4, ncols=1, sharex=True)
    ...
    ax2[0].legend()
    ax2[0].set_xticks(np.arange(0,95,5))
    ax2[0].grid(True)
    ...
    ax2[1].legend()
    ax2[1].set_xticks(np.arange(0,95,5))
    ax2[1].grid(True)
    ...
    ax2[2].set_ylabel('Heading Error (rad)')
    ax2[2].set_ylim([-0.75, 0.75])
    ax2[2].legend()
    ax2[2].set_xticks(np.arange(0,95,5))
    ax2[2].grid(True)

    # plot measurement availability
    meas_avail = 1.*np.isfinite(l_depth)

    l_time = np.squeeze(l_time)
    for rr in range(0, meas_avail.shape[0]):
        ax2[3].plot(l_time, meas_avail[rr,:], label="Landmark {}".format(rr))

    ax2[3].set_yticks([0, 1])
    ax2[3].set_ylabel('Landmark availability')
    ax2[3].set_xticks(np.arange(0,95,5))
    ax2[3].grid(True)
    # ax2[3].legend()
